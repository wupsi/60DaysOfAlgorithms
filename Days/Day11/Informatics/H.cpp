...
struct BinarySearchTree{
    ...
    bool isBalanced(Node *current){
        ...
        if (abs(height(current->right) -  height(current->left)) <= 1 and
            isBalanced(current->right) and isBalanced(current->left))
            return true;
        return false;
    }
};
...
int main(){
    ...
    int leftRoot = BST.height(BST.root->left);
    int rightRoot = BST.height(BST.root->right);
